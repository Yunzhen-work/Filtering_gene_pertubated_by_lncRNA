...
//2019.08.29 改动了第三步的筛选条件，根据ceRNA假设来筛选（lncRNA负影响microRNA，lncRNA正影响mRNA）

namespace 扰动lncRNA筛选
{
    class Program
    {
        static string workingDirectory = @"C:\Users\weiyu\Downloads\workspace\";

        //如果有第三步保存的文件就填写这个，没有的话就改成空，也就是连续两个引号
        static string step3OutputFilePath = "";
        //static string step3OutputFilePath = workingDirectory + "";

        static string lncRNAExpressionFilePath = workingDirectory + "LUAD-lncRNA-purity.txt";
        static string microRNAExpressionFilePath = workingDirectory + "LUAD-microRNA-purity.txt";
        static string mRNAExpressionFilePath = workingDirectory + "LUAD-mRNA-purity.txt";
            ...
            bool assignPrimaryKey = true)
        {
            ...
            while (notFinishReading)
            {
                try
                {
                    ...
                    //header
                    row_read = reader.ReadLine().Replace("'", "prime").Replace("\"", "");
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            //Todo: 改成microRNA的表达符合ceRNA的假设：lncRNA↑ microRNA↓
            ...
            if (step3OutputFilePath == "")
            {
                ...
                foreach (DataRow tripletRow in triplet.Rows)
                {
                    ...
                    //old: microRNA要求无差异表达，若有差异则不要，或已被IQR筛选掉了也不要
                    //new: microRNA要求有差异表达，并且highGroup在前lowGroup在后的时候要返回值logFC > +0.58
                    if (IsMicroRNADifferentiallyExpressed != "Positively")
                        continue;
                    ...
                    //old: mRNA要求有差异表达，若无差异则不要，或已被IQR筛选掉了也不要
                    //new: mRNA要求有差异表达，并且highGroup在前lowGroup在后的时候要返回值logFC < -0.58
                    if (IsMRNADifferentiallyExpressed != "Negatively")
                        continue;
                ...
                {
                    writer.WriteLine(string.Join("\t", dataRow.ItemArray).Replace("prime", "'"));
                }
